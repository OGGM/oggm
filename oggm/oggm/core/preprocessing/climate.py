...
def process_ccsm_data(gdir):
    ...
    loc_tmp = xr.concat([loc_tmp[9:], loc_tmp[:-3]], dim='month')
    ts_tmp = temp #+ loc_tmp
    ...
    loc_pre = xr.concat([loc_pre[9:], loc_pre[:-3]], dim='month')
    ts_pre = precp
    for j in range(12):
    ...
    time2 = np.concatenate((time1[9:], time1[:-3]), axis=0)
    ...
    #"back to -180 - 180"
    loc_lon = precp.lon if precp.lon <= 180 else precp.lon - 3
