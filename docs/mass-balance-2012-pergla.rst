...
anymore, now that we can calibrate on each glacier individually. Furthermore,
:math:`P_f` can now be calibrated on each glacier individually (**new in v1.6!**).
...
.. admonition:: Individual glacier :math:`P_f` calibtration using winter precipitation.
   :class: note, dropdown
...
correction. More exigent users might have a look
